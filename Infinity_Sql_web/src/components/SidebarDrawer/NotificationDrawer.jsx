...
const NotificationDrawer = ({ isOpen, onClose }) => {
  ...
  const handleChangeStatus = async(id, refPost) =>{
    try {
      ...
      if(refPost !== null) {
        await dispatch(getPostById(refPost.id));
        navigate(`/post/${refPost.id}`);
      }
        ...
        <div className="searched-user ">
          {notifications
            ? notifications.map((element, index) => {
                const {id, sender, message, unread, refPost=null, tag } = element;
                return (
                  <div className={`${unread ? 'unread' : ''} noti-users`} key={index} onClick={() => handleChangeStatus(id, refPost)}>
